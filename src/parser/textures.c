...
int	parse_texture_line(t_game_info *game, char *line)
{
	...
	if ((line[i] == 'W' && line[i + 1] == 'E' && is_whitespace(line[i + 2])))
		return (change_tex_status(game, i, line, WE));
	if ((line[i] == 'E' && line[i + 1] == 'A' && is_whitespace(line[i + 2])))
...
int	change_tex_status(t_game_info *game, int i, char *line, t_directions tex)
{
	...
	if (game->has_tex[tex] != 0)
		return (1);
	game->tex[tex] = ft_strtrim(line + st, " \t\n");
	if (!game->tex[tex] || game->tex[tex][0] == '\0')
		return (1);
	if (!check_file(game->tex[tex], ".xpm"))
		return (1);
	game->has_tex[tex] = 1;
	return (0);
}
