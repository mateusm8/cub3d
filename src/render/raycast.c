...
/*   Updated: 2026/09/21 18:54:14 by nalfonso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init_perWall(t_data *data,int side, double *perpWallDist, double sideDistX, double sideDistY)
{
	if (side == 0)
...
void	distanceCalculation(t_game *g, t_data *data, int side, double sideDistX, double sideDistY, int x)
{
	...
	perpWallDist = 0.0;
	init_perWall(data, side, &perpWallDist, sideDistX, sideDistY);
	// if (side == 0)
	// 	perpWallDist = sideDistX - data->deltaDistX;
	// else if(side == 1)
	// 	perpWallDist = sideDistY - data->deltaDistY;
